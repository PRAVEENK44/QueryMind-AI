...
def init_database():
    ...
    # HR Data
    depts = [("Engineering", "NAMER", 5000000), ("Sales", "EMEA", 3000000), ("Marketing", "APAC", 2000000), ("Support", "NAMER", 1500000)]
    ...
    cursor.executemany("INSERT INTO campaigns (name, channel, start_date, end_date, budget, roi_percent) VALUES (?, ?, ?, ?, ?, ?)", camp_data)
    ...
    cursor.executemany("INSERT INTO customers (company_name, industry, campaign_source_id, total_ltv) VALUES (?, ?, ?, ?)", cust_data)
    ...
    for _ in range(2000):
        ...
        int_data.append((c_id, e_id, random.choice(["Call", "Email", "Ticket"]), date, random.uniform(1.0, 10.0)))
    cursor.executemany("INSERT INTO interactions (customer_id, emp_id, type, date, sentiment_score) VALUES (?, ?, ?, ?, ?)", int_data)

    # Logistics & Products
    wh_data = [("New York Hub", 50000, 10), ("London Central", 45000, 20), ("Tokyo Apex", 30000, 30)]
    cursor.executemany("INSERT INTO warehouses (location, capacity, manager_emp_id) VALUES (?, ?, ?)", wh_data)
    ...
    cursor.executemany("INSERT INTO products (supplier_id, name, category, unit_cost, msrp) VALUES (?, ?, ?, ?, ?)", prod_data)

    inv_data = []
    for p_id in range(1, 51):
        for w_id in range(1, 4):
            inv_data.append((w_id, p_id, random.randint(50, 1000), 100))
    cursor.executemany("INSERT INTO inventory (warehouse_id, product_id, quantity_on_hand, restock_threshold) VALUES (?, ?, ?, ?)", inv_data)
    ...
    for o_id in range(1, 3001):
        ...
        if status != "Cancelled":
            ...
            del_date = (date_obj + timedelta(days=random.randint(4, 10))).strftime("%Y-%m-%d") if status == "Completed" else None
        ...
        paid = (date_obj + timedelta(days=random.randint(5, 45))).strftime("%Y-%m-%d") if status == "Completed" else None
        inv_docs.append((o_id, date, due, paid, "Paid" if paid else "Pending", order_total))

    cursor.executemany("INSERT INTO orders (customer_id, sales_rep_emp_id, date, total_amount, status) VALUES (?, ?, ?, ?, ?)", order_data)
    cursor.executemany("INSERT INTO order_items (order_id, product_id, quantity, subtotal) VALUES (?, ?, ?, ?)", item_data)
    cursor.executemany("INSERT INTO shipments (order_id, warehouse_id, dispatch_date, delivery_date, status) VALUES (?, ?, ?, ?, ?)", ship_data)
    cursor.executemany("INSERT INTO invoices (order_id, issue_date, due_date, paid_date, status, amount) VALUES (?, ?, ?, ?, ?, ?)", inv_docs)
