...
class IntentParserCache:
    ...
    def get_intent(self, query: str, previous: str | None = None) -> Any | None:
        """Get cached intent."""
        key = self._make_key(query, previous)
        return self.cache.get(key)

    def set_intent(self, query: str, intent: Any, previous: str | None = None):
        """Cache intent."""
        key = self._make_key(query, previous)
        self.cache.set(key, intent)

    def _make_key(self, query: str, previous: str | None) -> str:
        """Make cache key."""
