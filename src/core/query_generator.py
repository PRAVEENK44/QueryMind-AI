...
class QueryGenerator:
    ...
    def _get_required_tables(self, intent: QueryIntent, schema_info: dict) -> set:
        ...
        preference = None
        if "product" in str(intent.metric).lower() or (
            intent.group_by and "product" in str(intent.group_by).lower()
        ):
            preference = "products"
        elif "user" in str(intent.metric).lower() or (
            intent.group_by and "user" in str(intent.group_by).lower()
        ):
            preference = "users"
    ...
    def _find_table(
        self, column: str, schema_info: dict, preference: str | None = None
    ) -> str | None:
        ...
        for table in required_tables:
            ...
            for rel in relationships:
                if rel["from_table"] == "orders" and rel["to_table"] == table:
                    ...
                    joins.append(
                        f"LEFT JOIN {table} {t_alias} ON {f_alias}.{rel['from_col']} = {t_alias}.{rel['to_col']}"
                    )
                    break
    ...
    def _build_order_by(self, intent: QueryIntent) -> str:
        if intent.group_by:
            order_field = (
                intent.metric
                if intent.group_by != "date"
                else ("order_count" if intent.aggregation == "count" else intent.metric)
            )
