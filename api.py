...
def get_custom_core(session_id: str, db_path: str) -> dict:
    ...
    print(
        f"Custom DB core initialized for session {session_id}: {len(schema_info['tables'])} tables found.",
        flush=True,
    )
    return _custom_cores[session_id]
...
def force_primitive(v):
    ...
    try:
        ...
        if isinstance(v, np.ndarray):
            return [force_primitive(x) for x in v.tolist()]
    except ImportError:
        pass
    if isinstance(v, dict):
        return {str(k): force_primitive(val) for k, val in v.items()}
    if isinstance(v, (list, tuple, set)):
        return [force_primitive(x) for x in v]
    return str(v)
...
async def get_atlas():
    ...
    try:
        ...
        edges = [
            {"from": r["from_table"], "to": r["to_table"]} for r in schema.get("relationships", [])
        ]
...
async def get_dashboard():
    ...
    try:
        ...
        if not llm_dashboard:
            return JSONResponse(
                status_code=503,
                content={"success": False, "error": "Nexus Brain is currently offline."},
            )
        ...
        for c in llm_dashboard.get("charts", []):
            try:
                ...
                if res.success and res.data:
                    fig = core["viz_generator"].generate_chart(
                        res.data, {"metric": c.get("label")}, chart_type=c.get("type", "bar")
                    )
                    if fig:
        ...
        response_data = force_primitive(
            {"success": True, "kpis": kpis, "dashboard_charts": dashboard_charts}
        )
...
async def upload_database(file: UploadFile | None = None):
    ...
    try:
        ...
        os.remove(db_path)
        raise HTTPException(status_code=422, detail=f"Could not read database schema: {e}") from e
    ...
    return {
        ...
        "schema": {t: list(info["columns"].keys()) for t, info in schema_info["tables"].items()},
    }
        ...
        try:
            ...
            if isinstance(raw_filters, dict):
                filters_obj = QueryFilters(
                    **{k: v for k, v in raw_filters.items() if k in QueryFilters.model_fields}
                )
            else:
                filters_obj = QueryFilters()
            intent = QueryIntent(
            ...
            intent = QueryIntent(
                metric=intent_dict.get("metric", ""), sql_query=intent_dict.get("sql_query", "")
            )
